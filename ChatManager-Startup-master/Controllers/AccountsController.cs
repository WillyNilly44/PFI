...
namespace ChatManager.Controllers
{
    public class AccountsController : Controller
    {
        ...
        {
            var liste = DB.Chat.ToList().Where(u => u.IdReceive == userid || u.IdSend == userid);
            var listeFriends = DB.FriendShips.ToList().Where(i => i.IdAmis1 == userid || i.IdAmis2 == userid);
            foreach(var i  in liste)
            {
                DB.Chat.Delete(i.Id);
            }
            foreach(var i in listeFriends)
            {
                DB.FriendShips.Delete(i.Id);
            }
        ...
        public ActionResult AdminMod(int id)
        {
            ...
            User userMod = DB.Users.FindUser(id); 
            if(userMod != null)
            {
                return View(userMod);
            }
        ...
        {
            User usermod = DB.Users.FindUser(user.Id);;
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    newEmail = user.Email;
                    user.Email = user.ConfirmEmail = usermod.Email;
                }
                if (DB.Users.Update(user))
                {
                    if (newEmail != "")
